...
// ---- CONTROLE DE TEMA: INTERRUPTOR COM IMAGEM LOCAL ----
const btnThemeToggle = document.getElementById('btnThemeToggle');
const imgThemeIcon = document.getElementById('imgThemeIcon');

btnThemeToggle.addEventListener('click', () => {
  document.body.classList.toggle('dark-mode');
  const isDarkMode = document.body.classList.contains('dark-mode');
  
  // Troca o arquivo de imagem local dinamicamente com transição
  imgThemeIcon.src = isDarkMode ? './icons/full-moon.png' : './icons/sun.png';
...
function carregarTemaSalvo() {
  chrome.storage.local.get({ theme: 'light' }, (result) => {
    if (result.theme === 'dark') {
      ...
      imgThemeIcon.src = './icons/full-moon.png';
    } else {
      document.body.classList.remove('dark-mode');
      imgThemeIcon.src = './icons/sun.png';
    }
  });
}
  ...
  chrome.storage.local.get({tarefas: []}, (result) => {
    ...
    tarefas.forEach((tarefa, index) => {
      ...
      tr.innerHTML = `
        ...
        <td style="color: var(--text-muted);">${tarefa.projetoDescricao || '-'}</td>
...
function calcularMinutosExtras(entradaStr, saidaStr) {
  ...
  if (minEntrada < limiteManha) {
    ...
    minutesExtras += (fimEscopoManha - minEntrada);
  }

  if (minSaida > limiteTarde) {
    const inicioEscopoTarde = Math.max(minEntrada, limiteTarde);
    minutesExtras += (minSaida - inicioEscopoTarde);
  }

  return minutesExtras;
}
...
document.getElementById('btnGerarRelatorio').addEventListener('click', () => {
  ...
  chrome.storage.local.get({tarefas: [], projetos: []}, (result) => {
    ...
    let minutosExtrasSomadosDoPeriodo = 0; 
    ...
    result.tarefas.forEach(t => {
      if (t.data >= dataInicio && t.data <= dataFim) {
        ...
        if (tipoRelatorio === 'extras') {
          ...
          minutosExtrasSomadosDoPeriodo += minutosCalculados; 
    ...
    if (tipoRelatorio === 'extras' && encontrouAlgumMinuto) {
      ...
      html += `
        <div style="border-top: 2px solid var(--border-color); margin-top: 12px; padding-top: 8px;"></div>
        <div class="item-resultado" style="background: var(--alert-bg); padding: 8px; border-radius: 6px; font-weight: bold; border: 1px solid var(--alert-border);">
          <span class="nome-cliente" style="color: var(--alert-text);">📊 TOTAL ACUMULADO:</span>
          <span class="tempo-cliente" style="color: var(--alert-time); font-size: 15px;">${hTotal}h ${mTotal > 0 ? mTotal + 'm' : '00m'}</span>
        </div>
      `;
    }
